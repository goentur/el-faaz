...
namespace Modules\Ukuran\Http\Controllers;
...
class UkuranController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            return DataTables::eloquent(Ukuran::select('id', 'nama'))
                ->addIndexColumn()
                ->addColumn('aksi', function (Ukuran $data) {
                    $kirim = [
                        'data' => $data,
    ...
    public function edit($id)
    {
        $kirim = [
            'attribute' => $this->attribute,
            'data' => Ukuran::select('id', 'nama')->find(dekrip($id)),
        ];
    ...
    public function update(Request $request, $id)
    {
        ...
        Ukuran::select('id')->find(dekrip($id))->update([
            'nama' => $request->nama,
        ]);
    ...
    public function destroy(Request $request)
    {
        ...
        if ($request->ajax()) {
            Ukuran::select('id')->find(dekrip($request->id))->delete();
    ...
    {
        if ($request->ajax()) {
            return DataTables::eloquent(Ukuran::onlyTrashed()->select('id', 'nama'))
                ->addIndexColumn()
                ->addColumn('aksi', function (Ukuran $data) {
                    $kirim = [
                        'data' => $data,
    ...
    public function memulihkan(Request $request)
    {
        ...
        if ($request->ajax()) {
            Ukuran::onlyTrashed()->select('id')->find(dekrip($request->id))->restore();
            return response()->json([
                'status' => true,
    ...
    public function permanen(Request $request)
    {
        ...
        if ($request->ajax()) {
            Ukuran::onlyTrashed()->select('id')->find(dekrip($request->id))->forceDelete();
            return response()->json([
                'status' => true,
